...
async function seed() {
  const db = await mysql.createConnection({
    ...
    database: process.env.DB_NAME     || 'SOI-2026-0052-mizyana',
  ...
  for (const m of MERCHANTS) {
    ...
    const [existM] = await db.execute('SELECT merchant_id FROM MERCHANT WHERE user_id = ?', [userId]);
    let merchantId;
    if (existM.length) {
      ...
      const [mr] = await db.execute(
        ...
        [userId, m.biz.name, m.user.email, m.user.phone, m.biz.address]
      );
      merchantId = mr.insertId;
    }

    const [existS] = await db.execute('SELECT service_id FROM SERVICE WHERE merchant_id = ?', [merchantId]);
    if (!existS.length) {
      for (const s of m.services) {
        await db.execute(
          'INSERT INTO SERVICE (merchant_id, service_name, description, price, duration_mins) VALUES (?,?,?,?,?)',
          [merchantId, s.name, s.desc, s.price, s.dur]
        );
      }
    }

    const [existP] = await db.execute('SELECT promo_id FROM PROMOTION WHERE merchant_id = ?', [merchantId]);
    let promoId = null;
    if (!existP.length) {
      for (const p of m.promos) {
        const [pr] = await db.execute(
          'INSERT INTO PROMOTION (merchant_id, title, description, discount_pct, start_date, end_date) VALUES (?,?,?,?,?,?)',
          [merchantId, p.title, p.desc, p.pct, p.start, p.end]
        );
        promoId = pr.insertId;
      }
    } else {
    ...
    const [existF] = await db.execute('SELECT listing_id FROM FEATURED_LISTING WHERE merchant_id = ?', [merchantId]);
    if (!existF.length) {
      await db.execute(
        'INSERT INTO FEATURED_LISTING (merchant_id, promo_id, title, description, is_visible) VALUES (?,?,?,?,1)',
        [merchantId, promoId, m.featured.title, m.featured.desc]
      );
    }
